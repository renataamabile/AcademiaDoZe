...
public class LogradouroRepository : BaseRepository, ILogradouroRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            string query =
                ...
                "cep = @Cep, nome = @Nome, bairro = @Bairro, " +
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                DatabaseType.Sqlite =>
                    $"{BaseSelectQuery} WHERE cidade = @Cidade COLLATE NOCASE ORDER BY bairro, nome",

                DatabaseType.SqlServer =>
                    $"{BaseSelectQuery} WHERE cidade COLLATE Latin1_General_CI_AI = @Cidade COLLATE Latin1_General_CI_AI ORDER BY bairro, nome",

                DatabaseType.MySql =>
                    $"{BaseSelectQuery} WHERE cidade COLLATE utf8mb4_general_ci = @Cidade COLLATE utf8mb4_general_ci ORDER BY bairro, nome",

                _ => throw new InfrastructureException(
                    "SGDB_NAO_SUPORTADO",
                    $"SGDB não suportado: {_databaseType}")
            };
