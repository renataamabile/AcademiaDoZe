        ...
        ColaboradorVinculo vinculo)
    {
        ...
        if (!Enum.IsDefined(vinculo))
        {
            notifications.Add(
                new Notification(
                    "Vinculo",
                    "VINCULO_COLABORADOR_INVALIDO"));
        }

        if (
            Enum.IsDefined(tipo) &&
            Enum.IsDefined(vinculo) &&
